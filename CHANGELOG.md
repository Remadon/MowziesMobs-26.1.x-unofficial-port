...
- Multiple simultaneous boss bars (e.g. spawning Frostmaw, Ferrous Wroughtnaut, and Umvuthi near each other) rendered stacked exactly on top of each other instead of in separate rows, because the Y-offset NeoForge is supposed to thread between successive boss bar draws within a frame wasn't actually advancing. Custom boss bars now track their own stacking offset instead of relying on that.
- Custom boss bar name text (e.g. "Ferrous Wroughtnaut") never rendered, because its text color used an alpha value of 0, which NeoForge 26.1.2's GUI text renderer treats as fully transparent and skips drawing entirely.
